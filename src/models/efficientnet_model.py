"""
EfficientNet Model for Caltech-101 Classification

This script implements EfficientNet-based deep learning models using transfer learning.
Supports EfficientNet-B0 through EfficientNet-B7.
"""
...
class EfficientNetClassifier(nn.Module):
    ...
    def __init__(self, model_name='efficientnet_b0', num_classes=102, pretrained=True, freeze_backbone=False):
        """
        Args:
            model_name: 'efficientnet_b0', 'efficientnet_b1', ..., 'efficientnet_b7'
        ...
        # EfficientNet has a classifier with one Linear layer
...
class Trainer:
    """
    Trainer class for EfficientNet models
    """
...
def main():
    ...
    """
    print("=" * 80)
    print("EfficientNet Training for Caltech-101")
    print("=" * 80)
    
    # Configuration
    config = {
        # Model
        'model_name': 'efficientnet_b0',  # efficientnet_b0 through b7
        'pretrained': True,
    ...
    output_dir = Path(f"results/runs/efficientnet_{model_name_clean}_bs{config['batch_size']}_lr{config['learning_rate']}")
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    metrics = evaluator.evaluate_and_save(
        ...
        y_probs=y_probs,
        history=history,
        model_name=f"EfficientNet {config['model_name']}",
        config=config
    )
